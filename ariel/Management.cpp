...
TSeqLocVector Management::FetchQuerySequence(NucleoString<> Nucleotxt){
	...
	// que se pueden ver en: http://www.ncbi.nlm.nih.gov/IEB/ToolBox/CPP_DOC/doxyhtml/structSDataLoaderConfig.html#af6c625b5340111f52d5b85183dc9b9f2
    	SDataLoaderConfig dlconfig(false, eUseBlastDbDataLoader);   
...
CSearchDatabase Management::FetchNucleoNDataBases(){
...
CSearchDatabase Management::FetchNucleoMegaDataBases(){
	// El primero argumento es el nombre de la base de datos.	
	// El segundo argumento es un EMoleculeType que representa el tipo de molecula, en este caso nucleotidos.
...
string management::loadString(string file){
	ifstream fe(file);
	string nucleotide;
	fe.getline(fe,nucleotide);
	fe.getline(fe,nucleotide);
	return nucleotide;
}

NucleoString management::load_nucleostr(string file){
	string nucleotido = this->loadString(file);
	NucleoString<ADN> a = NucleoString<ADN>(nucleotido);
	return a;	
}
